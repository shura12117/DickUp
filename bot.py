...
async def process_wank(interaction: discord.Interaction):
    ...
    try:
        # ВАЖНО: добавлен await перед db.get_user
        user_data = await db.get_user(str(user.id), user.name)
        
        if user_data and user_data.get('last_wank'):
            last_wank = datetime.fromisoformat(user_data['last_wank'])
            cooldown_delta = timedelta(minutes=COOLDOWN_WANK)
            if datetime.now() - last_wank < cooldown_delta:
                remaining = int((cooldown_delta - (datetime.now() - last_wank)).total_seconds())
        ...
        # ВАЖНО: добавлен await перед db.add_wank и db.update_username
        count = await db.add_wank(str(user.id), user.name)
        await db.update_username(str(user.id), user.name)
...
async def process_up(interaction: discord.Interaction):
    ...
    try:
        ...
        if user_data and user_data.get('last_up'):
            last_up = datetime.fromisoformat(user_data['last_up'])
            cooldown_delta = timedelta(minutes=COOLDOWN_UP)
            if datetime.now() - last_up < cooldown_delta:
                remaining = int((cooldown_delta - (datetime.now() - last_up)).total_seconds())
                minutes = remaining // 60
                seconds = remaining % 60
                return await interaction.followup.send(
                    f"️ Кулдаун! Подожди ещё **{minutes} мин {seconds} сек**.", 
        ...
        # ВАЖНО: добавлен await
        new_size = await db.add_size(str(user.id), growth, user.name)
        await db.update_username(str(user.id), user.name)
...
async def on_ready():
    ...
    # ВАЖНО: добавлен await перед db.get_total_users
    total_users = await db.get_total_users()
    logger.info(f"👥 Пользователей в БД: {total_users}")
    
    logger.info(f"⏱️ Кулдаун /дроч: {COOLDOWN_WANK} мин")
    logger.info(f"⏱️ Кулдаун /ап: {COOLDOWN_UP} мин")
...
async def help_command(interaction: discord.Interaction):
    ...
    try:
        embed = discord.Embed(
            ...
            description=(
                "**Команды:**\n\n"
                f"**/дроч** (или /дрочить, /подрочить) - +1 к счёту (кулдаун **{COOLDOWN_WANK} мин**)\n"
                f"**/ап** - вырастить арматуру на 1-10 см (кулдаун **{COOLDOWN_UP} мин**)\n"
